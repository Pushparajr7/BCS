...
class Knob1 extends ConsumerStatefulWidget {
  const Knob1({super.key});

  @override
  ConsumerState<Knob1> createState() => _Knob1State();
}

class _Knob1State extends ConsumerState<Knob1>
  ...
  Widget build(BuildContext context) {
    ref.listen<bool>(
        mapListProvider
            .select((state) => state[2]["steps"][0]["steps"][28]["state"]),
        (previous, next) {
      if (next) {
        setState(() {
          knobValue1 = 180;
          knobValue2 = 90;
          print(
              "ref==================>${ref.read(mapListProvider)[2]["steps"][0]["steps"][28]["state"]}");
          print(
              "state============> ${ref.read(mapListProvider)[2]["steps"][0]["steps"][28]["state"]}");
          _updateTween();
        });
      }
    });
